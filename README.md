...
- type : choose type among : `Numeric` (general type for numbers), `DecimalFloat` (floating-point number with high precision), `SmallInteger` (integer smaller than 30 000) `String` (text), `Date` (date in format dd.mm.yyyy. If date is a year only, make it integer - eventually will update to [ISO format](https://fr.wikipedia.org/wiki/ISO_8601)), `Boolean` (binary as 1/0 or True/False)
